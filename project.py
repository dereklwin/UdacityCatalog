...
from database_setup import Base, Cities, Destinations, Userdb
...
def fbconnect():
    ...
    app_id = json.loads(open('/var/www//UdacityCatalog/fb_client_secrets.json', 'r').read())['web']['app_id']
    app_secret = json.loads(
        open('/var/www//UdacityCatalog/fb_client_secrets.json', 'r').read())['web']['app_secret']
...
def getUserID(email):
    try:
        user = session.query(Userdb).filter_by(email=email).one()
        return user.id
    except:
        return None
...
def getUserInfo(user_id):
    try:
        user = session.query(Userdb).filter_by(id=user_id).one()
        return user
    except:
        return None
...
def createUser(login_session):
    newUser = Userdb(name=login_session['username'], email=login_session['email'], picture = login_session['picture'])
    session.add(newUser)
    session.commit()

    user = session.query(Userdb).filter_by(email=login_session['email']).one()
    return user.id
